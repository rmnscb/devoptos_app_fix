  ...
  stage('Code Quality Analysis') {
   ...
   post {
    always {
     ...
     recordIssues aggregatingResults: true, tools: [javaDoc(javadocDir: './target/site/apidocs'), checkStyle(pattern: '**/target/checkstyle-result.xml'), findBugs(pattern: '**/target/findbugsXml.xml', useRankAsPriority: true), pmdParser(pattern: '**/target/pmd.xml')]
    }
   }
  }
